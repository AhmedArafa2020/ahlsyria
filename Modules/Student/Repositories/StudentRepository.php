...
class StudentRepository implements StudentInterface
{
    ...
    public function update($request, $instructor, $slug)
    {
        try {
            ...
            if ($slug == 'general') {
                ...
                return $this->updateAllowedForums($request, $instructor->user);
            } else {
    ...
    public function updateAllowedForums($request, $user) 
    {
        $user->allowedForums()->sync($request->forums ?? []);
        return $this->responseWithSuccess(___('alert.Forums updated successfully.'));
    }
    ...
    public function updateProfile($request, $id)
    {
        ...
        try {
            ...
            if ($request->hasFile('cv_file')) {
                ...
                if ($upload['status']) {
                    $student->cv_file_id = $upload['upload_id'];
                } else {
            ...
            $user->phone = $request->phone;
            $user->date_of_birth = date_db_format($request->date_of_birth);
            ...
            Log::error('Student Create error ', ['error' => $th->getMessage()]);
            DB::rollBack(); // rollback database transaction
    ...
    // start social
    public function storeSocial($request, $id)
    {
        ...
        try {
            ...
            $studentModel->social_media_links = json_decode($request->social_media_links);
            $studentModel->save(); // save data in database table
            DB::commit(); // commit database transaction
            return $this->responseWithSuccess(___('alert.Social media links added successfully.')); // return success response
        } catch (\Throwable $th) {
            DB::rollBack(); // rollback database transaction
            return $this->responseWithError(___('alert.something_went_wrong_please_try_again'), [], 400); // return error response
        }
