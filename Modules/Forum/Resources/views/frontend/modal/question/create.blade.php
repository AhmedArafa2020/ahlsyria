                    ...
                    <form action="{{ @$data['url'] }}" method="post" id="modal_values" enctype="multipart/form-data">
                        ...
                        <div class="ot-contact-form mb-15 mt-6">
                            <label for="category" class="ot-contact-label mb-2"> {{ ___('forum.Category') }} <span
                                    class="text-danger">*</span></label>
                            <select class="modal_select2" name="category" id="category">
                                    <option value="{{ $data['categories']->id }}" selected>{{ $data['categories']->id }}</option>
                            </select>
                            @error('category')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        ...
                        <div class="ot-contact-form mb-24 forum-question position-relative">
                            <label for="question" class="ot-contact-label"> {{ ___('forum.Your Question') }} <span
                                    class="text-danger">*</span></label>
                            {{-- <textarea class="form-control h-100 custom-textarea summernote" id="question" name="question" rows="10"
                                placeholder="{{ ___('forum.Write Your Question') }}">{{ old('question') }}</textarea> --}}
                                <textarea class="form-control custom-textarea summernote" name="question" id="question" rows="5" placeholder=""></textarea>
                        </div>
                        @error('question')
