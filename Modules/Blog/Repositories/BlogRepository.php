...
class BlogRepository implements BlogInterface
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            if ($request->hasFile('meta_image_id')) {
                $upload = $this->uploadFile($request->meta_image_id, 'blog/meta_image/images', [[700, 700], [300, 220], [750, 400]], '', 'image'); // upload file and resize image 35x35
                if ($upload['status']) {
    ...
    public function update($request, $id)
    {
        ...
        try {
            ...
            // Meta imsge upload
            if ($request->hasFile('meta_image_id')) {
                $upload = $this->uploadFile($request->meta_image_id, 'blog/meta_image/images', [[700, 700], [300, 220], [750, 400]], @$target->meta_image_id, 'image'); // upload file and resize image 35x35
                if ($upload['status']) {
                    $target->meta_image_id = $upload['upload_id'];
                } else {
                    return $this->responseWithError($upload['message'], [], 400);
    ...
    public function getBlogs($q, $cat = null)
    {
        try {
            $result = $this->model->query()->active()->with('iconImage')->select('id', 'title', 'description', 'image_id','created_at');
            if(!empty($q))
            {
                $result = $result->where('title', 'like', '%' . $q . '%');
            }
            if(! empty($cat)) {
                $result->whereHas('category', function($query) use($cat) {
                    $query->where('id', $cat);
                });
            }
            return $result->latest()->paginate(24);
