...
//logo style in admin
// Function to toggle logo based on theme
...
// Call updateLogo whenever theme changes
updateLogo(activeTheme); // Initialize on page load

items.forEach((item) => {
    item.addEventListener('click', () => {
        const element = item.getAttribute('data-theme-style');
        updateLogo(element); // Update logo based on selected theme
    });
});
