...
function ajaxError(response) {
    ...
    if (errors) {
        $("#error_all").text('يحتوي الفورم اعلاه على اخطاء');
        ...
        Toast.fire({
            icon: 'error',
            title: jsonValue.message == 'Unauthenticated.' ? 'قم بتسجيل الدخول' : jsonValue.message
        });
    }
}
...
$(document).ready(function() {
    ...
    $('.interest-event-action').on('click', function () {
        ...
        $.ajax({
            url: '/frontend/event/interest/' + eventId,
            // data: data,
            cache: false,
            contentType: false,
            processData: false,
    ...
    $('.notification-dropdown').on('click', function () {
        console.log($('.notifications-btn').find('.count').text());
        if(parseInt($('.notifications-btn').find('.count').text()) == 0){
            return
        }
        ...
        $.ajax({
            url: '/frontend/event/read-all-notification',
            // data: data,
            cache: false,
            contentType: false,
            processData: false,
...
function submitMentoringRequest () {
    const form = document.getElementById("mentoring-request-form");
    const data = new FormData(form);
    $(".preloader").fadeIn("slow");
    $.ajax({
        url: '/frontend/mentoring/new',
        type: "POST",
        data: data,
        processData: false,
        contentType: false,
        success: function (data) {
            ...
            $('#mentoringModal').modal('hide');
            } else {
                form.innerHTML ="something_went_wrong"
                errorHandler(something_went_wrong);
            }
        },
