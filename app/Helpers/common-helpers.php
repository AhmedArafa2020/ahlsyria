...
function ___($key = null, $replace = [], $locale = null)
{
    ...
    $file_path = base_path('lang/ar/' . $file . '.json');
...
if (!function_exists('coursePreviewType')) {
    ...
    {
        return DB::table('statuses')->whereIn('id', [15, 16, 28])->get();
    }
}
...
if(!function_exists('convert_to_minutes'))
{
    function convert_to_minutes($time){
        $time = explode(':', $time);
        return (int)(($time[0]*60) + ($time[1]) + ($time[2]/60));
    }
}
...
{
    $html =  ('<img class="light_mode_logo" src="' . showImage(setting('light_logo'), 'dark-logo.svg') . '" alt="img">');
    $html .= (' <img class="dark_mode_logo" src="' . showImage(setting('dark_logo'), 'light-logo.svg') . '" alt="img">');
    echo $html;
}
function darkLogo($light = true)
{
    $html = ('<a href="' . url('/') . '"> <img src="' . showImage(setting('dark_logo'), 'light-logo.svg') . '" alt="img"></a>');
    echo $html;
}
