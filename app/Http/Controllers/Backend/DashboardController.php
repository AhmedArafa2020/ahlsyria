...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $data['students_content_creator'] = $this->user->model()->where('role_id','4')->where('work_field','Content Creator')->count();
            $data['students_web_programming'] = $this->user->model()->where('role_id','4')->where('work_field','Web Programming')->count();
            $data['students_graphic_design'] = $this->user->model()->where('role_id','4')->where('work_field','Graphic Design')->count();
            $data['students_other'] = $this->user->model()
                                        ->where('role_id','4')
                                        ->where('work_field','!=','Content Creator')
                                        ->where('work_field','!=','Web Programming')
                                        ->where('work_field','!=','Content Creator')
                                        ->where('work_field','!=','Graphic Design')
                                        ->count();
