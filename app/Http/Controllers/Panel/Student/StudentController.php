...
class StudentController extends Controller
{
    ...
    {
        $data['title'] = ___('student.Anas Profile'); // title
    ...
    public function courses(Request $request)
    {
        try {
            ...
            $data['enrolls'] = $this->enrollRepository->model()->where('user_id', Auth::id())->with('course:id,title,course_duration,course_category_id,slug,thumbnail,instructor_name')
                ->search($request)
                ->latest()
                ->paginate(10);
    ...
    public function courseLearn($slug, $lesson_id)
    {
        try {
            ...
            $data['enroll'] = $this->enrollRepository->model()->where('user_id', Auth::id())->where('approved', 1)->whereHas('course', function ($q) use ($slug) {
                $q->where('slug', $slug);
            })->with('course:id,title,slug,course_duration,created_by,requirements,outcomes,description,instructor_name', 'lessons')->first();
