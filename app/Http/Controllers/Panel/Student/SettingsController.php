...
use Modules\Student\Http\Requests\SocialMediaLinksRequest;
...
class SettingsController extends Controller
{
    ...
    public function addSocial()
    {
        ...
        try {
            $data['url'] = route('student.store.social'); // url
            $data['title'] = ___('course.social_media_links'); // title
            @$data['button']   = ___('student.Save & Update'); // button
            $data['student']            = $this->studentRepository->model()->where('user_id', auth()->user()->id)->first();
            $html = view('panel.student.partials.modal.social.create', compact('data'))->render(); // render view
            return $this->responseWithSuccess(___('alert.data_retrieve_success'), $html); // return success response
        } catch (\Throwable $th) {
            return $this->responseWithError(___('alert.something_went_wrong_please_try_again'), [], 400); // return error response
        }
    ...
    public function storeSocial(SocialMediaLinksRequest $request){
        try {
            $result = $this->studentRepository->storeSocial($request, auth()->user()->id);
            if ($result->original['result']) {
                return $this->responseWithSuccess($result->original['message']); // return success response
            } else {
                return $this->responseWithError($result->original['message'], [], 400); // return error response
