                    ...
                    <div class="grid-list-view d-flex flex-wrap justify-content-between align-items-center gap-15">
                        ...
                        <div class="d-flex flex-wrap justify-content-between align-items-center gap-15">
                            ...
                            <div class="search-tab">
                                ...
                                <button class="tab-btn btn-options @if (@$_GET['sort'] == 'latest') text-primary @endif"
                                    data-val="new">{{ ___('frontend.Latest') }}</button>
                                {{-- <button class="tab-btn btn-options" data-val="highest_price">{{
                                    ___('frontend.Highest Price') }}</button>
                                <button class="tab-btn btn-options" data-val="lowest_price">{{ ___('frontend.Lowest
                                    Price') }}</button> --}}
                                @if (@$_GET['type'])
                                <button class="tab-btn btn-options text-primary" data-val="{{$_GET['type']}}">{{
                                    Str::ucfirst($_GET['type']) }}</button>
                                @endif
                            </div>
