    ...
    <div class="col-xxl-12  col-xl-12 col-lg-12">
        <div class="admin-hero mb-24">
            ...
            <div class="btn-wrapper d-flex flex-wrap gap-10 mt-20">
                <a href="{{ route('student.course') }}" class="fill-btn-white  wow fadeInLeft" data-wow-delay="0.3s">{{
                    ___('student.Discover Courses') }}</a>
        ...
        <div class="col-xxl-4 col-xl-6 col-lg-12 col-md-5">
            <div class="video-img overly1 mb-10">
                ...
                <div class="video-icon video-icon2">
                    <a class="popup-video btn-icon" @if (@$data['student']->lastVisited->course->firstLesson) href="{{
                        route('student.course.learn', [@$data['student']->lastVisited->course->slug,
                        encryptFunction(@$data['student']->lastVisited->course->firstLesson->id)]) }}" @endif
        ...
        <div class="col-xxl-6 col-xl-6 col-lg-12 col-md-7">
            <div class="my-learning-summary">
                ...
                <p class="pear d-inline-block">{{ @$data['student']->lastVisited->course->lessons->count() }}
                    {{ ___('frontend.Lesson') }},
                    {{
                    minutes_to_hours(@$data['student']->lastVisited->course->lessons()->where('is_quiz',0)->sum('duration'))
                    }}</p>
                ...
                <div class="d-flex align-items-end">
                    <div class="btn-wrapper">
                        ...
                        $data['student']->lastVisited->course->id)->where('approved',1)->count() > 0)
                        <a @if (@$data['student']->lastVisited->course->firstLesson) href="{{
                            route('student.course.learn', [@$data['student']->lastVisited->course->slug,
                            encryptFunction(@$data['student']->lastVisited->course->firstLesson->id)]) }}" @endif
                        ...
                        @endif
                        @else
                        <a href="javascript:void(0)" class="btn-primary-outline w-100">{{ ___('student.No Lesson')
                            }}</a>
                        @endif
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-xl-12">
                        ...
                        <div class="activity-title mb-30 d-flex justify-content-between flex-wrap align-items-center">
                            ...
                            <a href="{{ route('student.course') }}" class="font-600 mb-10">{{ ___('student.My Learning')
                                }}</a>
                        </div>
                    </div>
                            ...
                            <div class="single-progress mb-20">
                                ...
                                <div class="d-flex justify-content-between mb-15">
                                    <p class="count text-tertiary">{{ ___('student.Total') }}
                                        {{ count(@$enroll->completed_quizzes ?? []) + count($enroll->completed_lessons
                                        ?? []) + count(@$enroll->completed_assignments ?? []) }}
                                        {{ ___('student.done') }}</p>
    ...
    <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-3 row-cols-xl-4 row-cols-xxl-5 g-24">
            ...
            <div class="my-single-courses white-bg position-relative radius-8 h-calc">
                <div class="video-img2 overly1">
                    ...
                    <div class="video-icon">
                        <a class="popup-video btn-icon" @if (@$enroll->course->firstLesson) href="{{
                            route('student.course.learn', [$enroll->course->slug,
                            encryptFunction($enroll->course->firstLesson->id)]) }}" @endif>
                            <img src="{{ url('frontend/assets/images/icon/play.svg') }}" alt="img">
                        </a>
                    </div>
                </div>
                <div class="course-caption">
                    <h4>
                        <a @if (@$enroll->course->firstLesson) href="{{ route('student.course.learn',
                            [$enroll->course->slug, encryptFunction($enroll->course->firstLesson->id)]) }}" @endif
        ...
        <div class="col m-auto">
            <div class="not-data-found text-center pt-50 pb-50">
                <img src="{{ @showImage(setting('empty_table'), 'backend/assets/images/no-data.png') }}" alt="img"
                    class="w-100 mb-20">
            </div>
        </div>
        @endforelse
