                ...
                <div class="custom-modal-body ">
                    <form action="{{ @$data['url'] }}" method="post" id="modal_values" enctype="multipart/form-data">
                                ...
                                <div class="input-group mb-2 mr-sm-2">
                                    <div class="input-group-prepend">
                                        <div class="input-group-text">
                                            <input type="{{ $data['question']->type == 1 ? 'checkbox' : 'radio' }}"
                                                class="form-check-input" name="answers[]" value="{{ $key }}" {{
                                                @$data['question']->answer && in_array($option,
                                            json_decode(@$data['question']->answer)) ? 'checked' : '' }}
                                            >
                                        </div>
                                    </div>
                        ...
                        <div class="btn-wrapper d-flex flex-wrap gap-10 mt-20">
                            <button class="btn-primary-fill submit_form_btn" type="button">{{ @$data['button']
                                }}</button>
                            <button class="btn-primary-outline close-modal" type="button">{{ ___('student.Discard')
                                }}</button>
                        </div>
                    </form>
                </div>
